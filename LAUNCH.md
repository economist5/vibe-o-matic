...
| `VIBEIFY_OPEN_ACCESS` | optional — set to `1` to enable | **Time-bound public promo flag.** When set to `"1"`, the test-mode bypass branch accepts `bypass=1` WITHOUT a password match — anyone can render free. The UI surfaces this by swapping the test-mode card's password input for a GVC-Day-themed banner and showing a top-of-page banner ("🎉 GVC Day Open Weekend"). Used for events like the 6/9/26 GVC Day weekend special. Unset / `0` → normal password-gated test mode resumes. Flip via Vercel env vars + redeploy; takes effect on next request. |
